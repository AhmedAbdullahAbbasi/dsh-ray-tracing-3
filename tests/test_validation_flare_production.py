...
class TestFlareProductionFitsAgreement(unittest.TestCase):
    def test_extracts_three_fits_and_catches_a_displaced_npz_event(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with fits.open(full_fits, mode="update") as hdul:
                hdul[0].header["SCATSHA"] = scattering.metadata["table_sha256"]
                hdul[0].header["ABSSHA"] = absorption.metadata["table_sha256"]
                hdul[0].header["GITHEAD"] = "fixture"
                hdul[0].header["GITDIRTY"] = False
                hdul[0].header["PYVER"] = "fixture"
                hdul[0].header["NPVER"] = "fixture"
                hdul[0].header["JAXVER"] = "fixture"
                hdul[0].add_checksum()
            ...
            material_arrays = {
                "physics_" + name: np.asarray(getattr(physics, name))
                for name in (
                    "energy_kev",
            ...
            base = {
                ...
                **material_arrays,
                ...
                "simulation_git_head": "fixture",
                "simulation_git_dirty": False,
                "simulation_python": "fixture",
                "simulation_numpy": "fixture",
                "simulation_jax": "fixture",
                "history_count": 2_500_000,
                "sky_x_edges_arcsec": [4.5, 5.5, 6.5],
                "sky_y_edges_arcsec": [-2.5, -1.5, -0.5],
            ...
            for field, value, check in (
                (
                    ...
                    600.0,
                    "fluence_and_bin_range_closure",
                ),
                (
                    "physics_scattering_cross_section_cm2_per_h",
                    np.zeros_like(base["physics_scattering_cross_section_cm2_per_h"]),
                    "embedded_material_values",
                ),
            ):
                with self.subTest(field=field):
                    np.savez(npz, **{**base, field: value})
                    bad = audit_run(npz, **options)
                    self.assertFalse(bad["model_checks"][check])
            ...
            with fits.open(full_fits, mode="update") as hdul:
                ...
                hdul[0].add_checksum()
            bad = audit_run(npz, **options)
            self.assertFalse(bad["fits_checks"]["primary_pixels"])
            with fits.open(full_fits, mode="update") as hdul:
                hdul[0].data[:] = base["total_fluence"].sum(axis=(0, 1))
                hdul[0].add_datasum()
                hdul[0].add_checksum()
                hdul["ENERGY_BINS"].data["LOW"][0] = 2.5
                hdul["ENERGY_BINS"].add_datasum()
                hdul["ENERGY_BINS"].add_checksum()
            bad = audit_run(npz, **options)
            self.assertFalse(bad["fits_checks"]["energy_bins_edges"])
