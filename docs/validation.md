...
| 7. Convolution | Scores a three-angle impulse and top-hat source; also checks exact fractional-bin convolution and its conditional Monte Carlo variance | Runs a tabulated post-peak decay through the real source sampler, voxel transport, scorer, and observer bins; compares two annular light curves with exact continuous-time impulse convolution | Local run had consistent time-bin residuals; rerun pending after correcting the fully contained window's numerical variance |
